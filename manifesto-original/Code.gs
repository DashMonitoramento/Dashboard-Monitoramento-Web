...
function upsertLinha_(sheet, campos, dado){
  ...
  const linha = linhaAlvo === -1 ? sheet.getLastRow() + 1 : linhaAlvo;
  // Evita que o Sheets converta sozinho os campos "data" (AAAA-MM-DD) ou
  // "hora" (HH:MM) em datas/horas de verdade — força essas células a texto
  // antes de escrever, senão o app pode receber um objeto de data em vez do
  // texto esperado na próxima leitura.
  ['data','hora'].forEach(nomeCampo=>{
    const idx = campos.indexOf(nomeCampo);
    if(idx !== -1) sheet.getRange(linha, idx + 1).setNumberFormat('@');
  });
  sheet.getRange(linha, 1, 1, campos.length).setValues([valores]);
}
...
function normalizarValor_(nomeCampo, valor){
  // Rede de segurança: se alguma célula ainda vier como objeto de data
  // (por exemplo, uma linha antiga escrita antes desta proteção existir),
  // converte de volta pro formato de texto que o app espera.
  if(valor instanceof Date){
    if(nomeCampo === 'data') return Utilities.formatDate(valor, Session.getScriptTimeZone(), 'yyyy-MM-dd');
    if(nomeCampo === 'hora') return Utilities.formatDate(valor, Session.getScriptTimeZone(), 'HH:mm');
...
function lerAba_(nomeAba, campos){
  ...
  return valores.map(linha => {
    const obj = {};
    campos.forEach((c, i) => obj[c] = normalizarValor_(c, linha[i]));
    return obj;
  }).filter(obj => obj.id !== '');
}
