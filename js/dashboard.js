...
const Dashboard = (() => {
  ...
  function renderControleCargasCards() {
    ...
    const mapaIds = {
      ...
      DISPONIVEL: 'cargas-count-disponiveis'
    };
  ...
  function renderControleCargasLista() {
    ...
    wrap.innerHTML = itens.map(item => {
      ...
      // hoje e foi adicionado de novo em Separado (2ª carga no mesmo dia) — sinaliza visualmente
      // sem precisar de card/etapa separada (ver "duplicado" em definirStatusCarga/
      // moverDisponibilidadeParaSeparacao, firebase-init.js). Ela pediu explicitamente pra deixar
      // acontecer essa duplicidade (mesma placa podendo já ter passado por Carregado antes).
      const classeNomeDuplicado = item.duplicado ? ' cargas-item__nome--duplicado' : '';
    ...
    wrap.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (acao === 'iniciar') await cargasDashFirebase.definirStatusCarga(placa, 'EM_SEPARACAO', rotaAtual);
          ...
          // Só existe no card Disponível — ainda não existe doc em statusCarga pra essa placa,
          // por isso precisa do lote atômico que também encerra a disponibilidade (ver
          // moverDisponibilidadeParaSeparacao, firebase-init.js).
